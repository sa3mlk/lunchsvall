...
from scrapers import delicerano, bryners, brandstation, delinorr
...
def parse_lunchguide():
	...
	d = date.today()
	cached_file = "lunchguide/%d-%02d-%02d.db" % (d.year, d.month, d.day)

	# Load from the previously saved cache
	try:
		with file(cached_file, "rb") as f:
			daily_specials = pickle.loads(f.read())
	except IOError as e:
		...
		# Add more specialized scrapers too
		daily_specials.append(bryners.get_daily_specials())
		daily_specials.append(brandstation.get_daily_specials())
		daily_specials.append(delicerano.get_daily_specials())
		daily_specials.append(delinorr.get_daily_specials())

		# Cache this day
		with file(cached_file, "wb") as f:
			f.write(pickle.dumps(daily_specials))
